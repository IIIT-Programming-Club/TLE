...
class User(
    namedtuple(
        "User",
        "handle firstName lastName country city organization contribution "
...
class Problem(
    namedtuple(
        "Problem", "contestId problemsetName index name type points rating tags"
    )
):
    __slots__ = ()
...
RanklistRow = namedtuple(
    "RanklistRow", "party rank points penalty problemResults"
)
...
class HandleInvalidError(TrueApiError):
    def __init__(self, comment, handle):
        super().__init__(
            comment, f"`{handle}` is not a valid Codeforces handle"
        )
...
class RatingChangesUnavailableError(TrueApiError):
    def __init__(self, comment, contest_id):
        super().__init__(
            comment,
            f"Rating changes unavailable for contest with ID `{contest_id}`",
        )
...
class contest:
    ...
    async def ratingChanges(*, contest_id):
        ...
        try:
            ...
            raise
        return [
            make_from_dict(RatingChange, change_dict) for change_dict in resp
        ]
    ...
    async def standings(
        *,
        ...
        problems = [
            make_from_dict(Problem, problem_dict)
            for problem_dict in resp["problems"]
        ]
        ...
        ranklist = [
            make_from_dict(RanklistRow, row_dict) for row_dict in resp["rows"]
        ]
...
class user:
    ...
    async def status(*, handle, from_=None, count=None):
        ...
        for submission in resp:
            submission["problem"] = make_from_dict(
        ...
        return [
            make_from_dict(Submission, submission_dict)
            for submission_dict in resp
        ]
