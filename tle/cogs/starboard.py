...
class Starboard(commands.Cog):
    ...
    async def on_raw_reaction_remove(self, payload):
        ...
        try:
            if cf_common.user_db.check_exists_starboard_message(
                payload.message_id
            ):
                starboard_message_id = (
                    cf_common.user_db.get_starboard_message_id(
                        payload.message_id
                    )
                )
    ...
    def prepare_embed(message):
        ...
        embed.set_footer(
            text=str(message.author), icon_url=message.author.avatar_url
        )
        return embed
    ...
    async def here(self, ctx):
        ...
        await ctx.send(
            embed=discord_common.embed_success("Starboard channel set")
        )
    ...
    async def remove(self, ctx, original_message_id: int):
        ...
        if rc:
            await ctx.send(
                embed=discord_common.embed_success("Successfully removed")
            )
        else:
            await ctx.send(
                embed=discord_common.embed_alert("Not found in database")
            )
