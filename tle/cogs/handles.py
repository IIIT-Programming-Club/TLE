...
def get_prettyhandles_image(rows, font):
    ...
    for pos, name, handle, rating in rows:
        ...
        handle = _trim(handle)
        color = rating_to_color(rating)
        draw_row(str(pos), name, handle, str(rating)
                 if rating else 'N/A', color, y)
...
class Handles(commands.Cog):
    def __init__(self, bot):
        ...
        self.font = ImageFont.truetype(
            constants.NOTO_SANS_CJK_BOLD_FONT_PATH, size=26)  # font for ;handle pretty
    ...
    async def _set(self, ctx, member, user):
        ...
        if user.rank == cf.UNRATED_RANK:
            ...
            roles = [role for role in ctx.guild.roles if role.name ==
                     user.rank.title]
            if not roles:
                raise HandleCogError(
    ...
    async def list(self, ctx, *countries):
        ...
        users = [(member, handle, rating)
                 for member, handle, rating in users if member is not None]
        if not users:
    ...
    async def pretty(self, ctx, page_no: int = None):
        ...
        ratings, in color.
        """
        user_id_cf_user_pairs = cf_common.user_db.get_cf_users_for_guild(
            ctx.guild.id)
        ...
        for user_id, cf_user in user_id_cf_user_pairs:
            ...
            if member == ctx.author:
                author_idx = idx
            rows.append((idx, member.display_name,
                         cf_user.handle, cf_user.rating))
    ...
    async def _update_ranks(self, guild):
        ...
        member_handles = [(guild.get_member(int(user_id)), handle)
                          for user_id, handle in res]
        member_handles = [(member, handle) for member,
                          handle in member_handles if member is not None]
    ...
    def _make_rankup_embed(guild, contest, change_by_handle):
        ...
        for start in range(0, len(rank_changes_str), _MAX_RATING_CHANGES_PER_EMBED):
            ...
            if start:
                ...
                embed = discord_common.cf_color_embed_fixed(
                    title=contest.name, url=contest.url, description=desc, seed=embed_color_seed)
        ...
        top_rating_increases_embed = discord_common.cf_color_embed_fixed(description='\n'.join(
            top_increases_str) or 'Nobody got a positive delta :(', seed=embed_color_seed)
    ...
    async def auto(self, ctx, arg):
        ...
        if arg == 'on':
            ...
            raise ValueError(
                f"arg must be 'on' or 'off', got '{arg}' instead.")
    ...
    async def _publish_now(self, ctx, contest_id):
        try:
            ...
            raise HandleCogError(
                f'Contest with id `{e.contest_id}` not found.')
