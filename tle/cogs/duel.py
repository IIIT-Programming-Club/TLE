...
DUEL_RANKS = (
    ...
    DuelRank(
        2600, 3000, "International Grandmaster", "IGM", "#FF3333", 0xFF0000
    ),
    DuelRank(
        3000, 10 ** 9, "Legendary Grandmaster", "LGM", "#AA0000", 0xCC0000
    ),
)
...
class Dueling(commands.Cog):
    ...
    async def register(self, ctx, member: discord.Member):
        ...
        await ctx.send(
            f"{member.mention} successfully registered as a duelist."
        )

    @duel.command(brief="Challenge to a duel")
    async def challenge(
        self, ctx, opponent: discord.Member, rating: int = None
    ):
        """Challenge another server member to a duel. You have to specify a
        rating for the problem. The challenge expires if ignored for 5 minutes."""

        if rating is None:
            raise DuelCogError(f"No rating specified")
    ...
    async def profile(self, ctx, member: discord.Member = None):
        ...
        def duel_to_string(duel):
            ...
            problem = cf_common.cache2.problem_cache.problem_by_name[
                problem_name
            ]
    ...
    @duel.command(
        brief="Print head to head dueling history", aliases=["versushistory"]
    )
    async def vshistory(
        self,
        ctx,
    ...
    async def ranklist(self, ctx):
        ...
        users = [
            (
                member,
                cf_common.user_db.get_handle(member.id, ctx.guild.id),
                rating,
            )
