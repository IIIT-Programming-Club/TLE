...
class CacheControl(commands.Cog):
    ...
    async def ratingchanges(self, ctx, contest_id="missing"):
        ...
        if contest_id == "all":
            ...
            count = (
                await cf_common.cache2.rating_changes_cache.fetch_all_contests()
            )
        elif contest_id == "missing":
            await ctx.send("This may take a while")
            count = (
                await cf_common.cache2.rating_changes_cache.fetch_missing_contests()
            )
        else:
        ...
        await ctx.send(
            f"Done, fetched {count} changes and recached handle ratings"
        )
