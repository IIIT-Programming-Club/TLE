...
# TODO: Create a better seeding way
async def create_tour(ctx, index):
    """Creates an instance of tournament at challonge"""
    global curr_tour

    challonge_user = await challonge.get_user(_USERNAME, _API)
    challonge_tour = await challonge_user.create_tournament(
        name=f"IIIT Programing Club Dueling Tournament {index}",
        url=f"progclub{index}",
    )

    curr_tour = challonge_tour

    users = [
        (ctx.guild.get_member(user_id), user_id)
    ...
    users = [
        ...
        and cf_common.user_db.get_handle(user_id, ctx.guild.id) is not None
    ]
...
class Tournament(commands.Cog):
    def __init__(self, bot):
        ...
        _USERNAME = environ.get("CHALLONGE_USERNAME")

        # TODO: Add check if _API and _USERNAME are not specified

        self.bot = bot
        self.converter = commands.MemberConverter()
        self.draw_offers = {}

    @commands.group(brief="Tournament commands", invoke_without_command=True)
    ...
    async def register(self, ctx):
        ...
        if rc == 0:
            raise DuelCogError("You are already a registered contestant")
        await ctx.send(
            f"Successfully registered {ctx.author.mention}"
            + " as a contestant."
        )
    ...
    @tour.command(brief="Ends the current tournament")
    @commands.has_any_role("Admin", "Moderator")
    async def end(self, ctx):
        ...
        cf_common.user_db.clear_registeration_table()
    ...
    async def challenge(self, ctx):
        ...
        if req_match is None:
            raise DuelCogError(f"{ctx.author.mention}, You have lost :(")
        ...
        if cf_common.user_db.check_tour_match(
            challenger_id
        ) or cf_common.user_db.check_tour_match(challenger_id):
            raise DuelCogError(
        ...
        if cf_common.user_db.check_tour_match(
            challengee_id
        ) or cf_common.user_db.check_tour_match(challengee_id):
            raise DuelCogError(
        ...
        seen2 = {
            name
            for userid in userids
            for name, in cf_common.user_db.get_match_problem_names(userid)
        }
        ...
        if cf_common.user_db.cancel_match(duelid, Duel.EXPIRED):
            await ctx.send(
                f"{ctx.author.mention}, your request to duel {opponent.display_name} has expired!"
            )

    @tour.command(brief="Decline a duel")
    async def decline(self, ctx):
        """Decline a challenge against you"""
        status = cf_common.user_db.get_tour_status()
        if status == 0:
            raise DuelCogError(f"Tournament is not going on :/")
        ...
        await ctx.send(
            f"{ctx.author.display_name} declined a challenge by {challenger.mention}."
        )
    ...
    async def withdraw(self, ctx):
        ...
        await ctx.send(
            f"{ctx.author.mention} withdrew a challenge to {challengee.display_name}."
        )
    ...
    async def accept(self, ctx):
        ...
        await ctx.send(
            f"Starting duel: {challenger.mention} vs {ctx.author.mention}",
            embed=embed,
        )
    ...
    async def draw(self, ctx):
        """Offer a draw if you both agree on a draw"""
        status = cf_common.user_db.get_tour_status()
        if status == 0:
            raise DuelCogError(f"Tournament is not going on :/")
    ...
    async def registered(self, ctx):
        ...
        users = [
            (member, cf_common.user_db.get_handle(member.id, ctx.guild.id))
            for member in users
            if member is not None
        ]
        users = [
            (member, handle) for member, handle in users if handle is not None
        ]
    ...
    async def invalidate(self, ctx):
        ...
        if (
            datetime.datetime.now().timestamp() - start_time
    ...
    async def _invalidate(self, ctx, member: discord.Member):
        """Forcefully declare an ongoing duel invalid."""
